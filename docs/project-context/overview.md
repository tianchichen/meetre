...
目标是一屏读完：顶部先放视角与必要输入，中段是一台落在首屏约 3/5～2/3 位置的天平，表情圆形下方紧跟一个「下一步」大按钮，页面最下方是时间结算。整页背景色就是当前状态（绿=可以开、蓝=有优化空间、橙=建议异步、红=调过头了），梁向更重的一侧压下去。四种底色对黑色油墨字的对比度都不低于 5.2:1。内容装得下就正好一屏；参会者输入、草稿或窄屏撑破一屏时，整页滚动，而不是裁掉内容或塞进一个看不见的内层滚动区。

页面的主次是刻意排的：会议名退成一行眉标题（它标识这场会，但不影响该不该开），判词是唯一的 h1，视觉上以天平圆形支点上方的对话框气泡呈现，并在下方接一颗不重复判词措辞的解释气泡；当前投入 / 合理投入、同步人数、会议时长和返还时间集中在底部结算区。原始 / 当前 / AI 推荐的完整三方对比收进「判断依据」，不再与结论争夺注意力。
...
视角切换是一等控件，不是设置项：它和「组织者视角 / 参会者视角」放在会议上下文同一行，不占导航位。组织者看的是全盘规划，参会者要知道「我该用哪种方式参与」，两者的评估内容和 action button 都不同。参会者视角的 h1 直接给结论（你需要到场 / 会前给输入就够了 / 会后接收结论就够了），红色状态例外，底线被破时两个视角都先说底线。

参会者面板位于页面顶部，只放两个输入，不放结果菜单：我是哪些角色（可多选，一人常兼多职），以及我的贡献是什么（现场决定或对齐分歧 / 提供信息或材料 / 只需要知道结论）。外层不再使用卡片边框，选项用浅背景和下沿表示选中；方式由这两项推导，末尾一行只留可核对的事实（几条同步议题与我相关、我占多少时间）。必要角色底线压过自我评估：被议程列为必要角色时，两个异步档带删除线并禁用——底线不是自我判断的事。

方式刻意只有三种，没有「只参加相关议题」：中途进出会打乱会议节奏，现实里也几乎没人真的执行，到场就意味着待完整场次。`attendeePlan.recommendedMode` 只作为「我的贡献」的初值，不是固定判词；旧值 `full` / `partial` / `input_then_leave` / `async` 仍被两个校验器接受并映射到三种方式。

人员与议题控件都长在天平上，不另设配置面板。左盘每个角色一排人形图标，一人一个，点击循环「会议中 → 异步知会 → 无需参与」；必要角色用字重加下划线、可有可无的角色用常规字重区分，不占用状态色（底色本身就在变，色相会被状态色吃掉）。拖动任意角色行首的 `⠿` 把手到另一个角色，会把整个源角色合并进去，多人角色也可以操作；宿主的第一个人形叠一枚角色计数徽标，下方出现「合并角色 / 拆开」。点把手后再点目标是触控与键盘后备。合并只减少人数和成本，不能绕过必要角色底线：`breaches()` 和 `repairBreaches()` 都走 `hostRoleOf()`，比的是宿主的同步人数。右盘每个议题一行，点标题切同步/异步，行内步进器按 5 分钟调整。证据、原始/推荐/当前对比、JSON 导入都收在「判断依据」浮层里。

「下一步」按视角 × 状态决定：组织者看状态（按建议调整 / 改发一条异步更新 / 发出这个邀请），参会者就是推导出来的那一种方式（确认到场 / 改成会前异步给输入 / 改成会后接收结论），异步两档带一个「按这个方式落地」次选——先看文案再决定要不要真改配置；到场态没有次选，落地就是维持现状。按钮位于表情圆形正下方，点击后就地展开可编辑文案再复制。换角色或换贡献会清空草稿：一份为旧结论写的文案不该留在框里。红色状态例外：两个视角都不生成可发出的文案，只提供确定性的「补回必要底线」。
